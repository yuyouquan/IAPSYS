...
IAPSYS（传音 APK 发布管理系统）是一个面向传音集团内部的应用发布管理平台，旨在规范化和自动化 Android APK 的通道发布流程。系统覆盖从发布申请、审核、物料准备、应用上架到灰度监控的全生命周期管理。
...
| 角色编码 | 角色名称 | 职责描述 |
|---------|---------|---------|
| R01 | 需求方 | 添加应用、提交通道发布申请，通常为应用开发或产品负责人 |
| R02 | 运营人员 | 通道发布审核（运营审核）、物料审核 |
| R03 | 管理层 | 创建班车（流程单）、通道发布审核（老板会签） |
| R04 | 物料上传者 | 物料上传与管理 |
| R05 | 上架操作员 | 应用上架操作 |
| R06 | 测试人员 | 业务内测验证与驳回 |
| R07 | 灰度监控员 | 灰度监控数据查看 |
| R08 | 管理员 | 拥有所有节点的操作权限，可创建班车 |

### 2.2 节点操作权限矩阵

| 节点 | R01（需求方） | R02（运营人员） | R03（管理层） | R04（物料上传者） | R05（上架操作员） | R06（测试人员） | R07（灰度监控员） | R08（管理员） |
|------|-----|-----|-----|-----|-----|-----|-----|-----|
| 创建班车 | - | - | **创建** | - | - | - | - | **创建** |
| 添加应用 | **添加** | - | **添加** | - | - | - | - | **添加** |
...
- **操作**：可执行该节点的特定操作（如驳回路由）
- **查看**：只读模式，查看已提交的数据
- 管理员（R08）拥有所有节点的完整操作权限
- 班车创建权限仅限管理层（R03）和管理员（R08）
...
IAPSYS 的核心业务流程是 **APK 发布流水线**，由 7 个顺序节点组成：
...
- 当前配置的老板列表：老板A (BOSS-001)、老板B (BOSS-002)
- 任一老板驳回：流程回退至「通道发布申请」
- 所有老板通过：自动推进至「物料上传」
...
| 顺序 | 节点类型 | 节点名称 | 默认负责人 |
|------|---------|---------|-----------|
| 1 | channel_apply | 通道发布申请 | 需求方（R01） |
| 2 | channel_review | 通道发布审核 | 运营人员（R02） |
| 3 | material_upload | 物料上传 | 物料上传者（R04） |
| 4 | material_review | 物料审核 | 运营人员（R02） |
| 5 | app_publish | 应用上架 | 上架操作员（R05） |
| 6 | biz_test | 业务内测 | 测试人员（R06） |
| 7 | gray_monitor | 灰度监控 | 灰度监控员（R07） |
