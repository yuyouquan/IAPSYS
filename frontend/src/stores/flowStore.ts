...
interface FlowState {
  ...
  createShuttle: (type: 'monthly' | 'temporary', year: number, month: number, suffix?: string, expiresAt?: string) => Promise<FlowRecord>;
...
export const useFlowStore = create<FlowState>()((set, get) => ({
  ...
  createShuttle: async (type, year, month, suffix, expiresAt) => {
    const result = await flowService.createShuttle(type, year, month, suffix, expiresAt);
    await get().fetchFlowList({ page: 1 });
    return result;
  },
  deleteShuttle: async (flowId) => {
    await flowService.deleteShuttle(flowId);
